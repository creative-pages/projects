...
if (isset($_POST['sign_in'])) {
    ...
    $result = mysqli_query($dbconn, "SELECT * FROM `students` WHERE `email` = '$emailusername' || `username` = '$emailusername'");
    if (mysqli_num_rows($result) == 1) {
        $row =  mysqli_fetch_assoc($result);
        if (password_verify($password, $row['password'])) {
            if ($row['status'] == 1) {
                $_SESSION['student_login'] = $row['username'];
                $_SESSION['student_id'] = $row['id'];
                header("Location: index.php");
            } else {
                $errorS = "Your status is not active.";
            }
                ...
                <div class="panel-content bg-scale-0">
                    <form method="POST" action="<?= $_SERVER['PHP_SELF']; ?>">
                        ...
                        <div class="form-group text-center">
                             ...
                             <a class="btn btn-block mt-sm" href="register.php">Register</a>
                        </div>
                    </form>
                </div>
            </div>
