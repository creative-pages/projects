            ...
            <div class="panel-content">
                <div class="table-responsive">
                    <table id="basic-table" class="data-table table-bordered table table-striped nowrap table-hover" cellspacing="0" width="100%">
                        ...
                        <tbody>
                            <?php 
                            $result = mysqli_query($dbconn, "SELECT `issue_books`.`books_id`,`issue_books`.`id`, `issue_books`.`book_issue_date`, `students`.`fname`, `students`.`lname`, `students`.`roll`, `students`.`phone`, `books`.`book_name`, `books`.`book_image`
                                FROM `issue_books`
                                INNER JOIN `students` ON `students`.`id` = `issue_books`.`student_id`
                                INNER JOIN `books` ON `books`.`id` = `issue_books`.`books_id`
                            ...
                            <tr>
                                ...
                                <td>
                                    <a class="btn btn-warning" href="return_book.php?id=<?= $row['id']; ?>&book_id=<?= $row['books_id'] ?>">Return</a>
                                </td>
                            </tr>
                            <?php
                            }
